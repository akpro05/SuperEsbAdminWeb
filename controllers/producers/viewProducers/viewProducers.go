...
func (c *ViewProducer) Get() {
	...
	defer func() {
		...
		if err != nil {
			...
			log.Println(beego.AppConfig.String("loglevel"), "Info", "ViewProducer Page Fail")
		} else {
			c.Data["DisplayMessage"] = ""
			c.TplName = "index.html"
			log.Println(beego.AppConfig.String("loglevel"), "Info", "ViewProducer  Page Success")
		}
		return
	}()
	...
	row, err := db.Db.Query(`select id, producer_name, email, access_code,  status  from public."producers" where id=$1`, AdminId)
	if err != nil {
		log.Println(beego.AppConfig.String("loglevel"), "Error", err)
		err = errors.New("Unable to get Producer data")
		sendFailureResponse(c, "Unable to get Producer data")
		return
	}
	defer sql.Close(row)
...
func sendFailureResponse(c *ViewProducer, message string) {
