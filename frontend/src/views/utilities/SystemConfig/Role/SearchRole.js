...
import { SEARCH_ROLE_URL, CREATE_ROLE_URL } from '../../../../UrlPath.js';
...
const SearchRole = () => {
  ...
  const searchUserBreadcrumbs = [
    <Breadcrumbs aria-label="breadcrumb">
      ...
      <Typography color="black">{t('serachrole')}</Typography>
    </Breadcrumbs>
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(SEARCH_ROLE_URL);
  ...
  const handleFormSubmit = async () => {
    try {
      const response = await axios.post(SEARCH_ROLE_URL, {
        ...formData,
  ...
  return (
    <>
      <MainCard
        title={<span style={{ color: 'black ', fontWeight: 'bold', fontSize: '24px' }}>{t('serachrole')}</span>}
      ...
      >
        <Card sx={{ overflow: 'hidden' }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', p: 2 }}>
            ...
            <Box sx={{ mx: 2 }} />
            <Button variant="contained" color="secondary" style={{ backgroundColor: 'rgb(13 32 97)' }} onClick={() => navigate('/Role/CreateRole')}>
              {t('createButton')}
            </Button>
          </Box>
        </Card>
              ...
              {(() => {
                ...
                if (rows.length === 0) {
                  ...
                  return filteredRows.slice(startIndex, endIndex).map((row) => (
                    <TableRow key={row.Id}>
                      <TableCell>
                        <Link to={`/Role/ViewRole/${row.Id}`} style={{ textDecoration: 'none' }}>
