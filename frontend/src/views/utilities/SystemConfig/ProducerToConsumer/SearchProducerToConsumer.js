...
import { CREATE_PROD2CONS_URL, SEARCH_PROD2CONS_URL } from '../../../../UrlPath.js';
...
const SearchProducerToConsumer = () => {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(wb, 'ProducerToConsumerData.xlsx');
  };
  ...
  const exportToPDF = () => {
    ...
    doc.save('ProducerToConsumerData.pdf');
  };
  ...
  const searchUserBreadcrumbs = [
    <Breadcrumbs aria-label="breadcrumb">
      ...
      <Typography color="black">{t('searchProducerToConsumer')}</Typography>
    </Breadcrumbs>
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(SEARCH_PROD2CONS_URL);
  ...
  const handleFormSubmit = async () => {
    try {
      const response = await axios.post(SEARCH_PROD2CONS_URL, {
        ...formData,
  ...
  const hardcodedServicesData = [
    { serviceName: 'Service Name 1', serviceURL: '/Service1' },
    { serviceName: 'Service Name 2', serviceURL: '/Service2' },
    { serviceName: 'Service Name 3', serviceURL: '/Service3' },
  ];
  ...
  return (
    <>
      <MainCard
        title={<span style={{ color: 'black ', fontWeight: 'bold', fontSize: '24px' }}>{t('searchProducerToConsumer')}</span>}
      ...
      >
        <Card sx={{ overflow: 'hidden' }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', p: 2 }}>
            ...
            <Box sx={{ mx: 2 }} />
            <Button variant="contained" color="secondary" style={{ backgroundColor: 'rgb(13 32 97)' }} onClick={() => navigate('/ProducerToConsumer/CreateProducerToConsumer')}>
              {t('createButton')}
            </Button>
          </Box>
        </Card>
          ...
          <Table id="tableId">
            <TableHead style={{ backgroundColor: '#125e1f' }}>
              <TableRow>
                ...
                <TableCell style={{ color: 'white', fontWeight: 'bold' }}>{t('producer')}</TableCell>
                <TableCell style={{ color: 'white', fontWeight: 'bold' }}>{t('consumer')}</TableCell>
                {/* <TableCell style={{ color: 'white', fontWeight: 'bold' }}>Subscribed Services</TableCell> */}
                <TableCell style={{ color: 'white', fontWeight: 'bold' }}>{t('statusLabel')}</TableCell>
                <TableCell style={{ color: 'white', fontWeight: 'bold' }}>{t('subscribedServices')}</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {rows
                ...
                .map((row) => (
                  <React.Fragment key={row.Id}>
                    <TableRow>
                      <TableCell>
                        <Link to={`/ProducerToConsumer/ViewProducerToConsumer/${row.Id}`} style={{ textDecoration: 'none' }}>
                      ...
                      <TableRow>
                        <TableCell colSpan={6}>
                          <Table size="small">
                            <TableHead>
                              <TableRow>
                                <TableCell sx={{ fontWeight: 'bold', background: '#814141', color: 'white' }}>{t('serviceName')}</TableCell>
                                <TableCell sx={{ fontWeight: 'bold', background: '#814141', color: 'white' }}>{t('serviceURL')}</TableCell>
                              </TableRow>
                            </TableHead>
                            <TableBody>
...
export default SearchProducerToConsumer;