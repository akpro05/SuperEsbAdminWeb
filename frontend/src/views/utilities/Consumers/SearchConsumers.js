...
const SearchConsumers = () => {
  ...
  const exportToExcel = () => {
    ...
    const customizedTableData = tableData.map((row) => ({
      ...
      'Email': row.Email,
      'Consumer Code': row.ConsumerCode,
      'Status': row.Status,
    ...
    XLSX.writeFile(wb, 'ConsumerData.xlsx');
  };
  ...
  const searchUserBreadcrumbs = [
    <Breadcrumbs aria-label="breadcrumb">
      ...
      <Typography color="black">{t('searchConsumer')}</Typography>
    </Breadcrumbs>
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/Consumers/SearchConsumers');
  ...
  const handleFormSubmit = async () => {
    try {
      const response = await axios.post('/Consumers/SearchConsumers', {
        ...formData,
  ...
  return (
    <>
      <MainCard
        title={<span style={{ color: 'black ', fontWeight: 'bold', fontSize: '24px' }}>{t('searchConsumer')}</span>}
      ...
      >
        <Card sx={{ overflow: 'hidden' }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', p: 2 }}>
            ...
            <Box sx={{ mx: 2 }} />
            <Button variant="contained" color="secondary" style={{ backgroundColor: 'rgb(13 32 97)' }} onClick={() => navigate('/Consumers/CreateConsumers')}>
              {t('createButton')}
            </Button>
          </Box>
        </Card>
          ...
          <Table id="tableId">
            <TableHead style={{ backgroundColor: '#125e1f' }}>
              <TableRow>
                ...
                <TableCell style={{ color: 'white', fontWeight: 'bold' }}>{t('consumerName')}</TableCell>
              ...
              {(() => {
                ...
                if (rows.length === 0) {
                  ...
                  return filteredRows.slice(startIndex, endIndex).map((row) => (
                    <TableRow key={row.Id}>
                      <TableCell>
                        {/* <a href={`/Consumers/ViewConsumers/${row.Id}`} style={{ textDecoration: 'none' }}>
                          <Typography variant="h5" color="error">
                            {row.Id}
                          </Typography>
                        </a> */}
                        <Link to={`/Consumers/ViewConsumers/${row.Id}`} style={{ textDecoration: 'none' }}>
                          <Typography variant="h5" color="error">
                            {row.PartialId}
                          </Typography>
                        </Link>
...
export default SearchConsumers;
