...
import { SEARCH_PRODUCERS_URL } from '../../../UrlPath.js';
...
// Data to copy or export to xlsx or pdf in table
  ...
  const customizedTableData = tableData.map((row) => ({
    ...
    'Producer Name': row.ProducerName,
    'Email': row.Email,
  ...
  XLSX.writeFile(wb, 'ProducerData.xlsx');
};
...
const handleSearchChange = (e) => {
  ...
  const searchUserBreadcrumbs = [
    <Breadcrumbs aria-label="breadcrumb">
      ...
      <Typography color="black">{t('searchProducers')}</Typography>
    </Breadcrumbs>
  ];
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(SEARCH_PRODUCERS_URL);
      ...
      setRows([]); // Set rows to an empty array on error
    }
  };
  ...
  const handleFormSubmit = async () => {
    try {
      const response = await axios.post(SEARCH_PRODUCERS_URL, {
        ...formData,
  ...
  return (
    <>
      <MainCard
      title={<span style={{ color: 'black ', fontWeight: 'bold', fontSize: '24px' }}>{t('searchProducers')}</span>}
      ...
      >
        <Card sx={{ overflow: 'hidden' }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', p: 2 }}>
            ...
            <Box sx={{ mx: 2 }} />
            <Button variant="contained" color="secondary" style={{ backgroundColor: 'rgb(13 32 97)' }} onClick={() => navigate('/Producers/CreateProducers')}>
              {t('createButton')}
            </Button>
          </Box>
        </Card>
          ...
          <Table id="tableId">
            <TableHead style={{ backgroundColor: '#125e1f' }}>
              <TableRow>
                ...
                <TableCell style={{ color: 'white', fontWeight: 'bold' }}>{t('producerName')}</TableCell>
                <TableCell style={{ color: 'white', fontWeight: 'bold' }}>{t('emailLabel')}</TableCell>
                <TableCell style={{ color: 'white', fontWeight: 'bold' }}>{t('accessCode')}</TableCell>
              ...
              {(() => {
                ...
                if (rows.length === 0) {
                  ...
                  return filteredRows.slice(startIndex, endIndex).map((row) => (
                    <TableRow key={row.Id}>
                      <TableCell>
                        <Link to={`/Producers/ViewProducers/${row.Id}`} style={{ textDecoration: 'none' }}>
